...
function testDatabaseConnection() {
    try {
        ...
        const result = db_1.default.prepare('SELECT 1 as test').get();
...
function getDatabaseStats() {
    ...
    for (const tableName of tables) {
        try {
            ...
            const sampleData = db_1.default.prepare(`SELECT * FROM ${tableName} LIMIT 3`).all();
            stats.push({
                tableName,
...
function testOrganizationData(userId) {
    try {
        const orgData = db_1.default.prepare(`
      SELECT 
        o.id as org_id,
        o.name as org_name,
        o.industry,
