...
const enterprisePDFParser_1 = require("./enterprisePDFParser");
class DocumentProcessor {
        ...
        try {
            ...
            switch (documentInfo.mimeType) {
                case 'application/pdf':
                    ...
                    try {
                        const enhancedPdfData = await (0, enterprisePDFParser_1.parseDocumentPDF)(fileBuffer, documentInfo.originalName);
                        ...
                        if (enhancedPdfData.extractedData?.tables?.length > 0) {
                            pdfContent += '\n\n=== EXTRACTED TABLES ===\n';
                            enhancedPdfData.extractedData.tables.forEach((table, index) => {
                        ...
                        if (enhancedPdfData.extractedData?.tags?.length > 0) {
                            pdfContent += '\n\n=== EXTRACTED PLC TAGS ===\n';
                            enhancedPdfData.extractedData.tags.forEach((tag) => {
    ...
    async processDocument(buffer, originalName) {
        const documentId = (0, uuid_1.v4)();
    ...
    async extractPLCDataFromText(content, filename) {
        const extractedData = {
            tags: [],
            tables: [],
            plcInfo: {}
        ...
        try {
            ...
            if (filename.endsWith('.csv') || content.includes(',')) {
                ...
                if (lines.length > 1) {
                    ...
                    const isTagDB = headers.some(h => h.toLowerCase().includes('tag') ||
    ...
    determineScope(matchText) {
        const lower = matchText.toLowerCase();
        if (lower.includes('global') || lower.includes('var_global'))
            return 'Global';
        if (lower.includes('input') || lower.includes('var_input'))
            return 'Input';
        if (lower.includes('output') || lower.includes('var_output'))
            return 'Output';
        if (lower.includes('local') || lower.includes('var_local'))
            return 'Local';
        return 'Local';
    }
    extractAddress(matchText) {
        const addressMatch = matchText.match(/%[IMQ][BWDX]?[\d.]+/);
        return addressMatch ? addressMatch[0] : '';
    }
    determineDirection(matchText) {
        const address = this.extractAddress(matchText);
        return this.determineDirectionFromAddress(address);
    }
    determineDirectionFromAddress(address) {
        if (!address)
            return 'Internal';
    ...
    extractIOSpecifications(content, extractedData, filename) {
        const ioPatterns = [
    ...
    extractSafetyInfo(content, extractedData, filename) {
    ...
    extractAlarmInfo(content, extractedData, filename) {
        const alarmPatterns = [
            /(Priority\s*[0-9])/gi,
