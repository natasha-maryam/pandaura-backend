...
exports.ProjectVersionsTable = void 0;
const index_1 = __importDefault(require("../index"));
class ProjectVersionsTable {
    // Create new version
    static async createVersion(data) {
        // Get current version number
        const versionStmt = index_1.default.prepare(`
      SELECT COALESCE(MAX(version_number), 0) as current_version 
      FROM project_versions 
      WHERE project_id = ?
    `);
        const { current_version } = versionStmt.get(data.project_id);
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
        const result = stmt.run(data.project_id, data.user_id, nextVersion, JSON.stringify(data.state), data.message || null, data.timestamp || Math.floor(Date.now() / 1000), data.is_auto ? 1 : 0);
        return result.lastInsertRowid;
    }
    // Get version history
    static async getVersionHistory(projectId) {
        const stmt = index_1.default.prepare(`
      SELECT * FROM project_versions 
      WHERE project_id = ? 
      ORDER BY version_number DESC
    `);
        const versions = stmt.all(projectId);
    ...
    static async getVersion(projectId, versionNumber) {
        const stmt = index_1.default.prepare(`
      SELECT * FROM project_versions 
      WHERE project_id = ? AND version_number = ?
    `);
        const version = stmt.get(projectId, versionNumber);
        if (!version)
            return null;
        return {
            ...version,
    ...
    // Restore to version
    static async restoreVersion(projectId, versionNumber, userId) {
