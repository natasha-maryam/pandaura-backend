...
router.get('/db-debug', async (req, res) => {
    try {
        ...
        if (isProduction) {
            // PostgreSQL queries
            try {
                const users = await database_adapter_1.db.getAllUsers();
            ...
            // SQLite queries
            try {
                const result = database_adapter_1.db.prepare('SELECT COUNT(*) as count FROM users').get();
...
router.get('/db-connection', async (req, res) => {
    try {
        if (process.env.VERCEL || process.env.NODE_ENV === 'production') {
            ...
            const connectionTest = database_adapter_1.db.prepare('SELECT 1 as test').get();
            res.json({
                status: 'healthy',
                database: 'sqlite',
...
router.get('/db', (req, res) => {
    try {
        ...
        tables.forEach(table => {
            try {
                const result = database_adapter_1.db.prepare(`SELECT COUNT(*) as count FROM ${table}`).get();
...
router.get('/db/tables', (req, res) => {
    try {
        const tablesInfo = database_adapter_1.db.prepare(`
      SELECT name, type 
      FROM sqlite_master 
    ...
    `).all();
        const detailedInfo = tablesInfo.map((table) => {
            try {
                // Get column info
                const columns = database_adapter_1.db.prepare(`PRAGMA table_info(${table.name})`).all();
                // Get row count
                const rowCount = database_adapter_1.db.prepare(`SELECT COUNT(*) as count FROM ${table.name}`).get();
                return {
                    name: table.name,
                    type: table.type,
...
router.get('/db/sample/:table', (req, res) => {
    ...
    try {
        const sampleData = database_adapter_1.db.prepare(`SELECT * FROM ${table} ORDER BY created_at DESC LIMIT ?`).all(limit);
        res.json({
            status: 'success',
            table,
