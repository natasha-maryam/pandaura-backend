...
export class SimpleCodeGovernor {
  private static readonly DOCUMENT_ANALYSIS_SYSTEM = `You are a comprehensive PLC system analyzer and code generator.
...
Make sure each file is MASSIVE and COMPLETE with no skeleton code, TODOs, or placeholders.`;
      ...
      const completion = await Promise.race([
        ...
        new Promise<never>((_, reject) => 
          setTimeout(() => reject(new Error('LLM call timeout after 180 seconds')), 180000)
        )
      ]) as any;
...
`
  ...
  static async generateFromDocument(specText: string, prompt: string): Promise<SimpleGovernorResult> {
