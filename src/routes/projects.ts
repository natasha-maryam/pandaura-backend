...
import { ProjectsTable, CreateProjectData, UpdateProjectData } from '../db/tables/projects';
...
router.post('/', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const newProject = ProjectsTable.createProject(projectData);
...
router.get('/', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const projects = ProjectsTable.getProjectsByUserId(userId);
...
router.patch('/:projectId', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (!ProjectsTable.checkProjectOwnership(projectId, userId)) {
    ...
    const updatedProject = ProjectsTable.updateProject(projectId, userId, updates);
...
router.put('/:projectId/autosave', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    const userId = req.user?.userId;
    if (!userId) {
      return res.status(401).json({ error: 'User not authenticated' });
    ...
    const success = ProjectsTable.updateAutosaveState(projectId, userId, autosaveState);
...
router.put('/:projectId/save', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    const userId = req.user?.userId;
    if (!userId) {
      return res.status(401).json({ error: 'User not authenticated' });
    ...
    const success = ProjectsTable.updateAutosaveState(projectId, userId, saveState);
...
router.delete('/:projectId', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const success = ProjectsTable.deleteProject(projectId, userId);
...
router.get('/:projectId/ownership', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    const userId = req.user?.userId;
    if (!userId) {
      return res.status(401).json({ error: 'User not authenticated' });
    ...
    const isOwner = ProjectsTable.checkProjectOwnership(projectId, userId);
